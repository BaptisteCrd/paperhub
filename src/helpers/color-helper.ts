export class ColorHelper {
    // Base Plan
    public static readonly basePlanStrokeColor= "#000000";
    public static readonly basePlanFillColor= "#c4c1be";

    // Partition
    public static readonly invalidPartitionStrokeColor= "#ff0000";
    public static readonly validPartitionStrokeColor= "#000000";

    // Custom Path/Object
    public static readonly customPathStrokeColor= "#7baac7";
    public static readonly customPathFillColor= "#7baac7";

    // Objects
    public static readonly bakeryStandFillColor= "green";
    public static readonly doorFillColor= "purple";
    public static readonly extinguisherFillColor= "red";
    public static readonly lampFillColor= "blue";
    public static readonly shelfFillColor= "orange";
    public static readonly storageBoxFillColor= "yellow";
    public static readonly suspendedLampsColor= "pink";
    public static readonly stairsColor= "brown";
    public static readonly displayPanelColor= "cyan";
    public static readonly checkoutColor= "#cc5500";

}
